...
work_dir <-"C:/Users/qy0704/Desktop/R_Tutorial"
setwd(work_dir)
...
#put the file in the workinf directory
...
# Generate noise covariates and add them in the data
set.seed(123)
...
# Only run on a subsample of the data only
# This is the main dataset used in this test file
set.seed(333)
...
# then "scale" each covariate by those values by subtracting the mean and dividing by the sd
...
# enable training, validation, and test sets.
...
# Creating different types of formulas
...
# set params in causal forest function

ncolx<-length(processed.scaled.train.1)-2
ncov_sample<-floor(ncolx/3)
  ...
  for (tree.index in 1:num.trees) {
    ...
    if (double.Sample) {
      tree.obj <- honest.causalTree(formula, data = dataTree, 
                                    ...
                                    split.Rule="CT", split.Honest= T, split.Bucket=T, 
                                    bucketNum = 5, 
                                    bucketMax = 100, cv.option="CT", cv.Honest=T, 
                                    minsize = nodesize, 

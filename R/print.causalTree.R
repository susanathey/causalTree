                        ...
                        digits = getOption("digits"), ...)
{
    ...
    effect <- if (!is.null(tfun)) {
        if (is.null(frame$effect2)) tfun(frame$effect, ylevel, digits)
        else tfun(frame$effect2, ylevel, digits)
    } else format(signif(frame$effect, digits))
    term <- rep(" ", length(depth))
    term[frame$var == "<leaf>"] <- "*"
    z <- labels(x, digits = digits, minlength = minlength, ...)
    n <- frame$n
    z <- paste(indent, z, n, format(signif(frame$dev, digits)), effect, term)

...
honest.predictionTree <- function(formula, data, weights, treatment, subset, 
  ...
  init<-c()
	...
	if (!is.null(xlevels))
		...
		#call rpart here instead
		ctfit <- rpart(as.formula(formula), data=data, method="class", 
		                         control=rpart.control(cp=0, minbucket=minsize.temp*2))
		
		print("after rpart call")
		return(ctfit)
		print("###")
		...
		ans <- honest.est.predictionTree(ans, est_X, est_wts, est_treatment, est_Y)

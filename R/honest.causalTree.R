#
#  The temporary honest re-estimation function.
#
...
honest.causalTree <- function(formula, data, weights, treatment, subset, 
    ...
    indx2 <- match(c("formula", "est_data", "est_weights", "est_subset", "est_treatment"),
                   names(Call), nomatch = 0L)
    ...
    if (!split.Bucket) {
        ...
        if (missing(bucketNum)) {
            ...
            bucketNum <- 3 * minsize
            # at least 3 * minsize obs in one bucket
        }
    ...
    if (split.Rule.int %in% c(1, 3, 5, 7)) {
        if (!missing(split.Honest)) {
            warning("No split.Honest choice for your chosen splitting rule.")
        }
    ...
    if (split.Honest == TRUE && split.Rule.int %in% c(2, 3, 6, 7)) {
        ...
        # split.Honest = False
        if (split.alpha != 1) 
            warning("For dishonest splitting, split.alpha should be set 1.");
        split.alpha <- 1
    }

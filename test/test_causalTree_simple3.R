#modify this code for 3 treats simple model: tbd
...
ntreats<-3
w<-sample(ntreats,nrow(xv),replace=TRUE)
w<-w-1
# w <- (w-0.5)*2 #-1 or 1
# w_ <- -w
w_ <-1-w #0 or 1, tbd: fix for >2 treats
# w<-w*0
# w_<-w*0+1
yvpo<-0*xv[,1:2]
yvpo[,1]<-w*X
...
tree <- causalTree(as.formula("y~x1+x2"), 
                   ...
                   bucketMax = bucketMax.temp, cv.option=cv.option.temp, cv.Honest=cv.Honest.temp, minsize = minsize.temp, 
                   split.alpha = split.alpha.temp, cv.alpha = cv.alpha.temp, xval=xvalvec, HonestSampleSize=nest, cp=0, ntreats)
opcpid <- which.min(tree$cp[,4])
opcp <- tree$cp[opcpid,1]
tree_prune <- prune(tree, cp = opcp) 

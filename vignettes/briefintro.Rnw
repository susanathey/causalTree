...
For \textbf{CT-H}, the honest version, the splitting objective risk function is $\widehat{\emse}_\tau(\cals^\train,\calp)$, and
\begin{align*}
-\widehat{\emse}_\tau(\cals^\train,\calp) & = 
...
\end{align*}
where $S^2_{\cals^\train_\control}(\ell)$ is the within-leaf variance on outcome $Y$ for $\cals^\train_\control$ in leaf $\ell$, and $S^2_{\cals^\train_\treat}(\ell)$ is the counter part for $\cals^\train_\treat$.\\
...
\begin{align*}
-\widehat{\emse}_\tau(\cals^\train,\calp, \alpha) &= 
...
For \textbf{fit-H}, the honest version, the risk function is $\widehat{\emse}_{\mu, W}(\cals^\train, \calp)$,
\begin{align*}
-\widehat{\emse}_{\mu, W}(\cals^\train,\calp) & = 
...
\begin{align*}
-\widehat{\emse}_{\mu, W}(\cals^\train,\calp, \alpha) & = 
